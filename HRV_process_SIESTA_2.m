...
% input file location
filepath = 'Z:\SIESTA\Data\';
sub_list_p = [filepath '\Baseline_manuscript_analysis\Demo_Baseline_manuscript'];
input_path = [filepath '\Raw\'];
output_path_level1 = [filepath 'Preprocessed\ANNE\HRV'];
sub_list = readtable([sub_list_p '\preprocessed_sublist.csv']);
...
%%
% finished fil1 1:17
for file = 17:1:length(sub_list{:,1})
%for file = 1
    ECG_path = [input_path strcat('\', char(sub_list{file, 1}),'\ANNE\',char(sub_list{file, 2}),'\transcribe\processed_chest_ecg.csv')];
    cd('Z:\SIESTA\Data\Baseline_manuscript_analysis\Baseline_Script\')
    ECG_dt = import_ECG(ECG_path);    
    subID = char(sub_list{file, 1});
    foldID = char(sub_list{file, 2});
    %try
    cd(output_path_level1);
    HRVparams = InitializeHRVparams_siesta('siesta', subID, foldID);
